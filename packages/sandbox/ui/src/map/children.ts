    ...
    Children.forEach(flatChildren, (child) => {
      if (isValidElement(child)) {
        ...
        for (const key of childrenMapKeys) {
          const { symbols, isMultiple, shouldRemoveContainer } = childrenMap[key]
          ...
          if (symbols.includes(childSymbol)) {
            ...
            if (shouldRemoveContainer) {
              childrenProps[key].push(...Children.toArray(child.props.children))
            } else {
        ...
        if (isDefined(childrenMapRestKey)) {
          const { isMultiple, shouldRemoveContainer } = childrenMap[childrenMapRestKey]
          ...
          if (shouldRemoveContainer) {
            childrenProps[childrenMapRestKey].push(...Children.toArray(child.props.children))
          } else {
